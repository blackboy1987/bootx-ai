 ...
 */
public class AiToolBoxUtils {
    private static final String TYPE = "aitoolbox";

    private static final String baseUrl="https://aitoolbox.cn//";
    ...
    public static List<AiToolCategory> category(){
        ...
        Document parse = Jsoup.parse(s);
        Element content = parse.getElementById("sidebar");
        Elements h4List = content.getElementsByTag("ul");
        Element first = h4List.first();
        Elements select = first.select("li.sidebar-item");
        for (Element h4 : select) {
            if (StringUtils.equalsAnyIgnoreCase(h4.text(),"友情链接")){
                continue;
            }
            AiToolCategory parentCategory = new AiToolCategory();
            Element a1 = h4.getElementsByTag("a").first();
            String id = a1.attr("data-change").replace("#term-","");
            parentCategory.setType(TYPE);
            parentCategory.setTypeId(TYPE+"_"+id);
            parentCategory.setName(a1.text());
            aiToolCategories.add(parentCategory);
            // 他的二级分类
            try {
                Element element = a1.nextElementSibling().nextElementSibling();
                Elements lis = element.getElementsByTag("li");
                for (Element li : lis) {
                    Elements a = li.getElementsByTag("a");
                    String id1 = a.attr("href").replace("#term-","").replace(id+"-","");
                    ...
                    Element select1 = parse.getElementById(a.attr("href").replace("#",""));
                    if(select1!=null){
                        String attr = select1.attr("data-link");
                        child.setOtherUrl(attr);
                    }
